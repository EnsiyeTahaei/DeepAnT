...
class DeepAnT:
    ...
    def visualize_results(
        ...
        Args:
            target_seq (np.ndarray): The target sequence with shape (num_samples, feature_dim)
            pred_seq (np.ndarray): The predicted sequence with shape (num_samples, feature_dim)
            anomalies (dict): Dictionary with keys "Feature_X" and values as list of anomalies.
            thresholds (List[float]): The calculated threshold values for each feature.
        """
        ...
        # Two columns per feature (left: target vs. prediction, right: detected anomalies)
        fig, axs = plt.subplots(self.feature_dim, 2, figsize=(14, 4 * self.feature_dim), sharex=False, sharey=False)
        ...
        for f_idx in range(self.feature_dim):
            ...
            line1 = axs[f_idx, 0].plot(
                time_steps[:boundary_idx], original_data[:boundary_idx, f_idx], 
                label=input_label, color="blue", linewidth=1.5
            )
            line2 = axs[f_idx, 0].plot(
                time_steps[boundary_idx:], target_seq[:, f_idx], 
                label=target_label, color="green", linestyle='--', linewidth=1.5
            )
            line3 = axs[f_idx, 0].plot(
                time_steps[boundary_idx:], pred_seq[:, f_idx], 
                label=pred_label, color="orange", linestyle='-.', linewidth=1.5
            )
            ...
            if f_idx == 0:
                ...
                labels_left.extend(
                    [obj.get_label() for obj in (line1 + line2 + line3)]
                )
            ...
            line4 = axs[f_idx, 1].plot(
                time_steps[:boundary_idx], original_data[:boundary_idx, f_idx], 
                label=sequence_label, color="blue", linewidth=1.5
            )

            line5 = axs[f_idx, 1].plot(
                time_steps[boundary_idx:], target_seq[:, f_idx], color="blue", linewidth=1.5
            )

            split_line = axs[f_idx, 1].axvline(
                x=boundary_idx, color="red", linestyle='--', linewidth=1, label=split_label
            )
            ...
            if isinstance(anomalies, dict):
                ...
                if feature_key in anomalies:
                    ...
                    scatter_plt = axs[f_idx, 1].scatter(
                        [time_steps[boundary_idx + i] for i in anomaly_indices], 
                        target_seq[anomaly_indices, f_idx], color="red", 
